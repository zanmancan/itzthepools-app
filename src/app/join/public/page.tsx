...
export default function JoinPublicPage({
  ...
  return (
    <div className="mx-auto w-full max-w-3xl px-4 py-8">
      <h1 className="mb-4 text-2xl font-semibold">Join League (Public Link)</h1>

      {!token ? (
        <div className="rounded-md border border-red-800 bg-red-900/30 px-3 py-2 text-sm text-red-300">
          Missing token. Check your link or ask for a new public link.
        </div>
      ) : (
        <InviteFlowClient token={token} variant="public" />
      )}
    </div>
  );
}
