...
export default function SignupClient() {
  ...
  const doSignup = useCallback(async () => {
    ...
    setError(null);
    try {
      devlog("[signup] submit", { email, next });
  ...
  function onSubmit(e: React.FormEvent) {
    e.preventDefault();
    void doSignup();               // <- no-misused-promises friendly
  }
  ...
  const doVerify = useCallback(async () => {
    ...
    setError(null);
    try {
      devlog("[signup] OTP verify", { email });
  ...
  // Auto-submit OTP when 6 digits present
  useEffect(() => {
    if (sent && otp.replace(/\D/g, "").length === 6 && !busy) {
      void doVerify();
    }
  }, [sent, otp, busy, doVerify]);
      ...
      {!sent ? (
        <form onSubmit={onSubmit} className="space-y-3">
          ...
          <input
            ...
            required
          />
          <button className="rounded bg-blue-600 px-4 py-2 disabled:opacity-50" disabled={busy}>
        ...
        <div className="rounded border border-neutral-700 p-4">
          <p className="mb-2 text-sm">
            ...
            After you click it, you’ll be returned here to finish joining the league.
          </p>
          <ul className="ml-5 list-disc text-sm text-gray-300">
            <li>Click the link in the email (opens a new tab). You’ll be signed in.</li>
          ...
          <div className="mt-3 flex items-center gap-2">
            ...
            />
            <button
              onClick={() => void doVerify()}   // <- wrapper avoids warning
