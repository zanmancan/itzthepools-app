...
import { supabase } from "../supabase"; // keeping your original import path
...
export default function Dashboard() {
  ...
  const load = useCallback(async () => {
    ...
    const { data, error } = await supabase
      .from("profiles")
      .select("id, email, team_name, avatar_url")
      .eq("id", user.id)
      .single();
  ...
  useEffect(() => {
    // ensure the async work is not treated as a floating promise
    void load();
  }, [load]);
      ...
      {profile && (
        <div className="card mb-6">
          ...
          <p className="opacity-80">
            Create a private league or use an invite link to join one.
          </p>
        </div>
      )}
